<template>
	<view class="tree">
		<scroll-view :scroll-y="true">
			<template v-for="(leaf,index) in list">
...
<script>
	export default {
		props: {
			...
			isAllfold: { //是否全部折叠 0:折叠 1：展开	 2：按选择
				type: Number,
				default: 2
			},
			isMultiple: {
		...
		methods: {
			//递归将一个树型转换为单层列表
			levelTrees(trees = [], level = 0, parentId = []) {
				for (const leaf of trees) {
					this.list.push({
						id: leaf.id,
						name: leaf.name,
						level,
						parentId, //父级id数组
						isFold: leaf.isFold ? true : false, //缺省折叠
						isMultiple: leaf.isMultiple ? true : false,
						isSelected: item.isSelected ? true : false || selected.includes(item.id) ?
							true : false, //是否是挑选中的
					});
				}
			}
